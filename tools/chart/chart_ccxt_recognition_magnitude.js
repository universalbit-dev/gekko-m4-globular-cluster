...
const MACRO_SIGNAL_LOG = path.join(LOG_DIR, '../logs/macro_signal.log');
...
async function loadLatestTfModel(tfFrame) {
  ...
  try {
    ...
    console.error(`[ERROR] Could not load TensorFlow model for ${tfFrame}:`, err);
    return null;
  }
}
...
function loadConvNetModel() {
  ...
  try {
    ...
    console.error(`[ERROR] Failed to load ConvNet model:`, e);
    return null;
  }
}
...
function predictConvNet(net, candle) {
  try {
    ...
    console.error("[ERROR] ConvNet prediction error:", e);
    return "N/A";
  }
}
...
function predictTensorFlow(model, candle) {
  try {
    ...
    console.error("[ERROR] TF prediction error:", e);
    return "N/A";
  }
}
...
async function runRecognitionMulti() {
  ...
  for (const tf of TIMEFRAMES) {
    ...
    try {
      ...
      console.log(`[chart_c] Wrote enhanced prediction JSON: ${outPath}`);
    } catch (e) {
...
async function intervalRunner() {
  ...
  setInterval(runRecognitionMulti, MULTI_INTERVAL_MS);
}

// --- Execute recognition at interval ---
intervalRunner();
