...
const MODEL_DIR = path.resolve(__dirname, '../trained/trained_ohlcv');
const OUT_CSV_PATH = path.resolve(__dirname, '../logs/exchangesimulator_prediction.csv');
const SIGNAL_LOG_PATH = path.resolve(__dirname, '../logs/exchangesimulator_signal.log');

const LABELS = ['bull', 'bear', 'idle'];
const INTERVAL_SIMULATOR_MS = parseInt(process.env.INTERVAL_SIMULATOR_MS, 10) || 300000; // default 5m
...
// Initial run
runRecognition();
// Repeat every INTERVAL_SIMULATOR_MS (5 minutes by default)
setInterval(runRecognition, INTERVAL_SIMULATOR_MS);
