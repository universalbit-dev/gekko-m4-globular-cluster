...
LOGFILE="${CLEAN_OHLCV_LOGFILE:-$SCRIPT_DIR/clean_ohlcv.log}"

# Ensure the directory for the log file exists and LOGFILE is non-empty
mkdir -p "$(dirname "$LOGFILE")"
if [[ -z "${LOGFILE:-}" ]]; then
  echo "LOGFILE is empty; aborting." >&2
  exit 2
fi

# Export variables used by the background child process.
# The child is started with bash -c, which does not inherit non-exported shell variables.
export SCRIPT_DIR EXPLORER_JS NODE_CMD PIDFILE LOGFILE
...
run_clean_once() {
  ...
  if [[ -z "${NODE_CMD:-}" ]]; then
    echo "[$(date -u +"%Y-%m-%dT%H:%M:%SZ")] ERROR: NODE_CMD is empty or node not found in PATH" | tee -a "$LOGFILE"
    return 1
  fi
...
start_daemon() {
  ...
  if [[ -f "$PIDFILE" ]]; then
    ...
    if kill -0 "$pid" >/dev/null 2>&1; then
      ...
      echo "Stale PID file found, removing." | tee -a "$LOGFILE"
      rm -f "$PIDFILE"
    fi
  fi
  ...
  # Start the daemon in background and capture its PID in PIDFILE.
  # We export the required variables above so the child can see them.
  # We also include a trap to exit cleanly on SIGTERM inside the child.
  # Use bash -lc with declare -f to pass function definitions into the child.
  (
  ...
  sleep 0.2
  if [[ -f "$PIDFILE" ]]; then
    echo "Daemon started (PID $(<"$PIDFILE"))" | tee -a "$LOGFILE"
  else
...
stop_daemon() {
  ...
  pid=$(<"$PIDFILE")
  if ! kill -0 "$pid" >/dev/null 2>&1; then
    echo "Process $pid not running. Removing PID file." | tee -a "$LOGFILE"
    rm -f "$PIDFILE"
    return 1
  fi
  echo "Stopping PID $pid..." | tee -a "$LOGFILE"
  kill "$pid"
  ...
  if kill -0 "$pid" >/dev/null 2>&1; then
    echo "PID $pid did not exit, sending SIGKILL..." | tee -a "$LOGFILE"
    kill -9 "$pid"
  fi
  rm -f "$PIDFILE"
  echo "Stopped." | tee -a "$LOGFILE"
}
...
status() {
  if [[ -f "$PIDFILE" ]]; then
    ...
    pid=$(<"$PIDFILE")
    if kill -0 "$pid" >/dev/null 2>&1; then
      echo "Daemon running (PID $pid)." | tee -a "$LOGFILE"
      echo "Log: $LOGFILE" | tee -a "$LOGFILE"
      return 0
    else
      echo "PID file exists but process $pid not running." | tee -a "$LOGFILE"
      return 1
    fi
  else
    echo "Daemon not running." | tee -a "$LOGFILE"
    return 3
  fi
}
