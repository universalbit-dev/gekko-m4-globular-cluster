...
// incoming WS:
wss.on('connection', ws => {ws.on('message', _.noop);});
app
  .use(cors())
...
server.listen(config.api.port, config.api.host, '::', () => {
  var host = `${config.ui.host}:${config.ui.port}${config.ui.path}`;
  //ssl
  if(config.ui.ssl == true) {
  var host = `${config.ui.host}:${config.ui.port}${config.ui.path}`;
}
  console.log('Serving Gekko-M4 UI on ' + host +  '\n');
  ...
  if(!isDevServer && !config.headless) {
    opn(host)
      .catch(err => {
        console.log('Something went wrong when trying to open your web browser. UI is running on ' + host + '.');
    });
  }
});
