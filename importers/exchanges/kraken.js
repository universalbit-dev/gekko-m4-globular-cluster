...
var fetch = () => {
    ...
    if (lastId) {
        ...
        setTimeout(() => {fetcher.getTrades(tidAsTimestamp, handleFetch)}, 500);
    }
    else
...
var handleFetch = (err, trades) => {
    ...
    var last = moment.unix(_.last(trades).date).utc();lastId = _.last(trades).tid
    
    if(last < from) {log.debug('Skipping data, they are before from date', last.format());
        return fetch();
    }

    if  (last > end || lastId === prevLastId) {fetcher.emit('done');
